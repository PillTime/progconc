...
public abstract class BQueueTest {
  ...
  void test6(int capacity) {
    ...
    AtomicInteger c = new AtomicInteger();
    CSystem.forkAndJoin(
        () -> { a.set(q.remove()); b.set(q.remove()); c.set(q.remove()); },
        () -> { q.add(1); },
        () -> { q.add(10); },
        () -> { q.add(100); }
        );
  ...
  void test7(int capacity) {   
    ...
    CSystem.forkAndJoin(
        () -> { q.add(1); q.add(2); q.add(3); q.add(4); a.set(q.size()); },
        () -> { b.set(q.remove()); c.set(q.remove()); d.set(q.size()); }
        );
    ...
    CSystem.forkAndJoin(
        () -> { a.set(q.size()); },
        () -> { b.set(q.remove()); q.add(6); c.set(q.remove()); d.set(q.size()); }
        );
    assertEquals(0, q.size());
